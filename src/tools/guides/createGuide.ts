...
import type { CreateGuideRequest } from '../../api/types/guide.js';
import { z } from 'zod';
...
const paramsSchema = z.object({
  type: z.enum(['shipping', 'transport', 'devolution']).default('shipping')
    .describe('The type of guide to create'),
  date: z.string().describe('Guide date (YYYY-MM-DD)'),
  due_date: z.string().describe('Due date (YYYY-MM-DD)'),
  loaded_at: z.string().describe('Loaded date and time (YYYY-MM-DD HH:MM or ISO 8601)'),
  address_from: addressSchema.describe('Origin address'),
  address_to: addressSchema.describe('Destination address'),
  client: clientSchema.describe('Client information'),
...
export const createGuideTool: McpTool = {
  name: 'guide_create',
  description: 'Create a new guide (shipping, transport, or devolution)',
  inputSchema: {
    type: 'object',
    required: ['date', 'due_date', 'loaded_at', 'address_from', 'address_to', 'client', 'items'],
    properties: {
      type: { 
        type: 'string', 
        enum: ['shipping', 'transport', 'devolution'],
        default: 'shipping',
        description: 'The type of guide to create'
      },
      ...
      address_from: {
        type: 'object',
        required: ['detail', 'city', 'postal_code', 'country'],
        properties: {
