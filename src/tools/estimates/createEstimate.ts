...
import type { CreateEstimateRequest } from '../../api/types/estimate.js';
import { z } from 'zod';
...
const paramsSchema = z.object({
  type: z.enum(['quote', 'proforma', 'fees_note']).default('quote')
    .describe('The type of estimate to create'),
...
export const createEstimateTool: McpTool = {
  name: 'estimate_create',
  description: 'Create a new estimate (quote, proforma, or fees note)',
  inputSchema: {
    type: 'object',
    ...
    properties: {
      type: { 
        ...
        default: 'quote',
        description: 'The type of estimate to create'
      },
  ...
  handler: async (args, server) => {
    ...
    const result = await server.estimatesEndpoint.create(estimateData);
